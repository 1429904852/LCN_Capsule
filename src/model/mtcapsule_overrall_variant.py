...
from src.module.capsule_variant import CapsLayer_variant
import tensorflow as tf
from src.module.nn_layers import bi_dynamic_gru
from src.utils.data_helper import margin_loss


class multiCapsule_Overrall_Variant(object):
    def __init__(self, max_sen_len, n_class_1, n_class_2, n_class_3, embedding_document, embedding_dim,
        ...
        with tf.name_scope('level_2_cap'):
            ...
            with tf.variable_scope('FeatCap_SemanCap_2'):
                SemanCap_variant = CapsLayer_variant(label=self.predict_11, batch_size=batch_size, num_outputs=self.sc_num, vec_len=self.sc_dim,
                                     iter_routing=self.iter_routing, with_routing=False, layer_type='CONV')
                self.caps2 = SemanCap_variant(input=self.hidden_level_2, kernel_size=self.filter_size, stride=1,
                                      embedding_dim=2 * self.hidden_size + self.n_class_1)

            with tf.variable_scope('ASC_ClassCap_2'):
                ASC_ClassCap_variant = CapsLayer_variant(label=self.predict_11, batch_size=batch_size, num_outputs=self.n_class_2, vec_len=self.cc_dim,
                                         iter_routing=self.iter_routing,
                                         with_routing=True, layer_type='FC')
                self.ASC_caps_2 = ASC_ClassCap_variant(self.caps2)
        ...
        with tf.name_scope('level_3_cap'):
            ...
            with tf.variable_scope('FeatCap_SemanCap_3'):
                SemanCap_variant = CapsLayer_variant(label=self.predict_22, batch_size=batch_size, num_outputs=self.sc_num, vec_len=self.sc_dim,
                                     iter_routing=self.iter_routing,
                                     with_routing=False, layer_type='CONV')
                self.caps3 = SemanCap_variant(input=self.hidden_level_3, kernel_size=self.filter_size, stride=1,
                                      embedding_dim=2 * self.hidden_size + self.n_class_2)

            with tf.variable_scope('ASC_ClassCap_3'):
                ASC_ClassCap_variant = CapsLayer_variant(label=self.predict_22, batch_size=batch_size, num_outputs=self.n_class_3, vec_len=self.cc_dim,
                                         iter_routing=self.iter_routing,
                                         with_routing=True, layer_type='FC')
                self.ASC_caps_3 = ASC_ClassCap_variant(self.caps3)
