    ...
    def __init__(self, max_sen_len, n_class_1, n_class_2, n_class_3, embedding_document, embedding_dim,
        ...
        with tf.name_scope('level_2_cap'):
            ...
            with tf.variable_scope('FeatCap_SemanCap_2'):
                SemanCap_2 = CapsLayer(batch_size=batch_size, num_outputs=self.sc_num, vec_len=self.sc_dim,
                                     iter_routing=self.iter_routing,
                                     with_routing=False, layer_type='CONV')
                self.caps2 = SemanCap_2(input=self.hidden_level_2, kernel_size=self.filter_size, stride=1,
                                      embedding_dim=2 * self.hidden_size + self.n_class_1)

            with tf.variable_scope('ASC_ClassCap_2'):
                ASC_ClassCap_2 = CapsLayer(batch_size=batch_size, num_outputs=self.n_class_2, vec_len=self.cc_dim,
                                         iter_routing=self.iter_routing,
                                         with_routing=True, layer_type='FC')
                # routing
                self.v_1 = tf.reshape(self.ASC_caps_1, [-1, self.n_class_1, self.cc_dim])
                self.v_1 = tf.expand_dims(self.v_1, -1)
                self.ASC_caps_2 = ASC_ClassCap_2(self.caps2, self.v_1)
        ...
        # with tf.variable_scope('Re_Routing_1'):
        #     self.v_1 = tf.reshape(self.ASC_caps_2, [-1, self.n_class_2, self.cc_dim])
        #     self.v_1 = tf.expand_dims(self.v_1, -1)
        #     self.ASC_caps_11 = ASC_ClassCap_1(self.caps1, mode=self.v_1)
        #     self.ASC_sv_length_11 = tf.sqrt(tf.reduce_sum(tf.square(self.ASC_caps_11), axis=2, keepdims=True) + 1e-9)
        #     self.predict_11 = tf.reshape(self.ASC_sv_length_11, [batch_size, self.n_class_1])
        ...
        with tf.name_scope('level_3_cap'):
            ...
            with tf.variable_scope('FeatCap_SemanCap_3'):
                SemanCap_3 = CapsLayer(batch_size=batch_size, num_outputs=self.sc_num, vec_len=self.sc_dim,
                                     iter_routing=self.iter_routing,
                                     with_routing=False, layer_type='CONV')
                self.caps3 = SemanCap_3(input=self.hidden_level_3, kernel_size=self.filter_size, stride=1,
                                      embedding_dim=2 * self.hidden_size + self.n_class_2)

            with tf.variable_scope('ASC_ClassCap_3'):
                ASC_ClassCap_3 = CapsLayer(batch_size=batch_size, num_outputs=self.n_class_3, vec_len=self.cc_dim,
                                         iter_routing=self.iter_routing,
                                         with_routing=True, layer_type='FC')
                ...
                self.ASC_caps_3 = ASC_ClassCap_3(self.caps3, self.v_2)
        ...
        # with tf.variable_scope('Re_Routing_2'):
        #     self.v_11 = tf.reshape(self.ASC_caps_3, [-1, self.n_class_3, self.cc_dim])
        #     self.v_11 = tf.expand_dims(self.v_11, -1)
        #     self.ASC_caps_111 = ASC_ClassCap_1(self.caps1, mode=self.v_11)
        #     # self.ASC_caps_22 = ASC_ClassCap_2(self.caps2, mode=self.v_2)
        #     self.ASC_sv_length_111 = tf.sqrt(tf.reduce_sum(tf.square(self.ASC_caps_111), axis=2, keepdims=True) + 1e-9)
        #     self.predict_111 = tf.reshape(self.ASC_sv_length_111, [batch_size, self.n_class_1])

        with tf.name_scope('loss'):
            reg_loss = tf.get_collection(tf.GraphKeys.REGULARIZATION_LOSSES)
            self.loss_1 = margin_loss(self.level_1, self.predict_1)
            # self.loss_11 = margin_loss(self.level_1, self.predict_11)
            # self.loss_111 = margin_loss(self.level_1, self.predict_111)
