...
class multiGRU(object):
    def __init__(self, max_sen_len, n_class_1, n_class_2, n_class_3, num_heads, embedding_document,
        ...
        with tf.name_scope('encoder'):
            ...
            # self.multi_head = tf.reshape(self.multi_head, [-1, self.max_sen_len, 2 * self.hidden_size])
            # [-1, 1, max_len] [-1, max_len, 2 * hidden]
            self.attention_1 = mlp_attention_layer(self.multi_head, self.sen_len, 2 * self.hidden_size, self.l2_reg,
                                                self.random_base, 1)
            ...
            # [-1, 500, n_class_1]
            self.level_11 = tf.tile(self.level_11, [1, self.max_sen_len, 1])

            # label injection
            # [-1, 500, n_class_1] [-1, n_class_1] [n_class_1, 1]
            # [-1, 500, 2 * hidden] [-1, 2 * hidden] [2 * hidden, 1]
            # [-1, 500] [-1, 500, 1]
            self.level_11 = tf.reshape(self.level_11, [-1, self.n_class_1])
            self.multi_head_1 = tf.reshape(self.multi_head, [-1, 2 * self.hidden_size])
            gate_1 = tf.nn.sigmoid(tf.matmul(self.level_11, self.weights['w_level_1']) + tf.matmul(self.multi_head_1, self.weights['w_head_1']))
            ...
            # label concat
            # # [-1, 500, 2 * hidden + n_class_1]
            # self.hidden_2 = tf.concat([self.multi_head, self.level_11], 2)
            # # [-1 * 500, 2 * hidden + n_class_1]
            ...
            # [-1, 2 * hidden]
            self.attention_2 = mlp_attention_layer(self.multi_head_2, self.sen_len, 2 * self.hidden_size, self.l2_reg,
                                                self.random_base, 2)
            ...
            self.level_22 = tf.tile(self.level_22, [1, self.max_sen_len, 1])

            # label injection
            self.level_22 = tf.reshape(self.level_22, [-1, self.n_class_2])
            self.multi_head_2 = tf.reshape(self.multi_head, [-1, 2 * self.hidden_size])
            gate_2 = tf.nn.sigmoid(tf.matmul(self.level_22, self.weights['w_level_2']) + tf.matmul(self.multi_head_2, self.weights['w_head_2']))
            ...
            # label concat
            # self.hidden_3 = tf.concat([self.multi_head_2, self.level_22], 2)
            ...
            self.attention_3 = mlp_attention_layer(self.multi_head_3, self.sen_len, 2 * self.hidden_size, self.l2_reg,
                                                self.random_base, 3)
        ...
        with tf.name_scope('loss'):
            ...
            # self.output_1 = tf.reshape(self.output_1, [-1, 2 * self.hidden_size])
            self.predict_1 = tf.matmul(self.output_1, self.weights['softmax_1']) + self.biases['softmax_1']
            self.predict_1 = tf.reshape(self.predict_1, [-1, self.n_class_1])
            self.predict_1 = tf.nn.softmax(self.predict_1)
            ...
            # self.output_2 = tf.reshape(self.output_2, [-1, 2 * self.hidden_size])
            ...
            # label re-routing
            # self.true_1_2 = tf.reshape(tf.matmul(self.level_1, self.weights['w_true_1']), [-1, self.n_class_2])
            # self.predict_2_2_norm = tf.sqrt(tf.reduce_sum(tf.square(self.predict_2_2)))
            # self.true_1_2_norm = tf.sqrt(tf.reduce_sum(tf.square(self.true_1_2)))
            # tensor1_tensor2 = tf.reduce_sum(tf.multiply(self.predict_2_2, self.true_1_2))
            # cosin_1 = tensor1_tensor2 / (self.predict_2_2_norm * self.true_1_2_norm)
            # self.predict_2_2 = self.predict_2_2 + self.true_1_2 * cosin_1
            ...
            # label re-routing
            # self.true_2_3 = tf.reshape(tf.matmul(self.level_2, self.weights['w_true_2']), [-1, self.n_class_3])
            # self.predict_3_3_norm = tf.sqrt(tf.reduce_sum(tf.square(self.predict_3_3)))
            # self.true_2_3_norm = tf.sqrt(tf.reduce_sum(tf.square(self.true_2_3)))
            # tensor2_tensor3 = tf.reduce_sum(tf.multiply(self.predict_3_3, self.true_2_3))
            # cosin_2 = tensor2_tensor3 / (self.predict_3_3_norm * self.true_2_3_norm)
            # self.predict_3_3 = self.predict_3_3 + self.true_2_3 * cosin_2
