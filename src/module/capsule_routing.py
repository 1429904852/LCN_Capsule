...
class CapsLayer(object):
    def __init__(self, batch_size, num_outputs, vec_len, iter_routing, with_routing=True, layer_type='FC'):
    ...
    def __call__(self, input, mode=None, kernel_size=None, stride=None, embedding_dim=None):
        if self.layer_type == 'CONV':
            ...
            if not self.with_routing:
                ...
                Semantic Capsules
                '''
                capsules = tf.expand_dims(context_conv, -1)
        ...
        with tf.variable_scope('iter_' + str(r_iter)):
            ...
            if r_iter == iter_routing - 1:
                ...
                if mode == None:
                    ...
                    # [batch_size, pc_num, sc_num_1, sc_dim, 1]
                    # [batch_size, pc_num, sc_num_2, sc_dim, 1]

                    # [batch_size, pc_num, 1, sc_dim, sc_num_1 + sc_num_2]
                    print("--------------------------------")
                    print(v_J_tiled.shape)
                    print(mode.shape)

                    # v_mode_l = tf.concat([v_J_tiled, mode], 2)
                    # v_mode_2 = tf.transpose(v_mode_l, [0, 1, 4, 3, 2])
                    #
                    # w_att = tf.get_variable(
                    #     name='w_att',
                    #     shape=(
                    #     1, pc_num, 1, mode.shape[2].value + u_hat_stopped.shape[2].value, u_hat_stopped.shape[2].value),
                    #     dtype=tf.float32,
                    #     initializer=tf.random_normal_initializer(stddev=0.01)
                    # )
                    # # [batch_size, pc_num, 1, sc_dim, sc_num_1 + sc_num_2]
                    # # [batch_size, pc_num, 1, sc_num_1 + sc_num_2, sc_num_1]
                    # # [batch_size, pc_num, 1, sc_dim, sc_num_1]
                    # # [batch_size, pc_num, sc_num_1, sc_dim, 1]
                    # # [batch_size, pc_num, sc_num_1, 1, 1]
                    # w_att = tf.tile(w_att, [batch_size, 1, 1, 1, 1])
                    # # [batch_size, pc_num, sc_num_1, 1, 1]
                    #
                    # gate = tf.sigmoid(
                    #     tf.reduce_sum(tf.transpose(tf.matmul(v_mode_2, w_att), [0, 1, 4, 3, 2]), axis=3, keepdims=True))
                    ...
                    # [batch_size, pc_num, 1, sc_dim, sc_num_2]
                    # [batch_size, pc_num, 1, sc_num_2, sc_num_1]
                    # [batch_size, pc_num, 1, sc_dim, sc_num_1]
                    # [batch_size, pc_num, sc_num_1, sc_dim, 1]
                    # [batch_size, pc_num, sc_num_1, sc_dim, 1]
                    # [batch_size, pc_num, sc_dim, sc_num_1, 1]
